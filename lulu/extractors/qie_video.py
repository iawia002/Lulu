...
class QieVideo(VideoExtractor):
    ...
    def extract(self, **kwargs):
        for stream_id in self.streams:
            self.streams[stream_id]['src'], dur = general_m3u8_extractor(
                self.streams[stream_id]['m3u8_url']
            )
            self.streams[stream_id]['video_profile'] += \
                ', Duration: {}s'.format(math.floor(dur))
