...
class MGTV(VideoExtractor):
    ...
    def download(self, **kwargs):
        ...
        if not kwargs['info_only']:
            if player:
                # with m3u8 format because some video player can process
                # urls automatically (e.g. mpv)
                launch_player(player, [stream_info['m3u8_url']])
            else:
                download_urls(
                    stream_info['src'], self.title, stream_info['container'],
                    stream_info['size'], output_dir=kwargs['output_dir'],
